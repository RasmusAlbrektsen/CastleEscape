...
public class DataMediator {
	 ...
	 */
	public void readLevel(String levelFolderPath) {
		...
		try {
			...
			//Read level files
			readInspectableObjects(saxParser, handler, levelFolderPath + INSPECTABLE_OBJECT_DIRECTORY);
			readItems(saxParser, handler, levelFolderPath + ITEM_DIRECTORY);
			readRooms(saxParser, handler, levelFolderPath + ROOM_DIRECTORY);

			//Finish building the rooms
			buildRoomExits();

			//Read config
			readConfig(saxParser, handler, levelFolderPath + CONFIG_FILE);
